...
class ImageProcessor {
  ...
  init() {
    // Create off-screen canvas for image processing (only in browser context)
    if (typeof document !== 'undefined') {
      this.canvas = document.createElement('canvas');
      this.ctx = this.canvas.getContext('2d');
      this.isServiceWorker = false;
    } else {
      // Service worker context - use OffscreenCanvas if available
      if (typeof OffscreenCanvas !== 'undefined') {
        this.canvas = new OffscreenCanvas(800, 600);
        this.ctx = this.canvas.getContext('2d');
        this.isServiceWorker = true;
      } else {
        console.log('⚠️ Canvas not available in service worker - using fallback methods');
        this.canvas = null;
        this.ctx = null;
        this.isServiceWorker = true;
      }
  ...
  // Helper method to check if image processing is available
  canProcessImages() {
    return !this.isServiceWorker && this.canvas && typeof Image !== 'undefined';
  }

  // Resize image to specified dimensions
  async resizeImage(imageData, maxWidth = 1024, maxHeight = 1024, quality = 0.9) {
    // In service worker context, return the image data as-is for now
    if (!this.canProcessImages()) {
      console.log('🔧 Image processing not available in service worker, returning original');
      return imageData;
    }
  ...
  async cropImage(imageData, cropArea) {
    // In service worker context, return original image
    if (!this.canProcessImages()) {
      console.log('🔧 Cropping not available in service worker, returning original');
      return imageData;
    }
  ...
  async enhanceImage(imageData, options = {}) {
    // In service worker context, return original image
    if (!this.canProcessImages()) {
      console.log('🔧 Image enhancement not available in service worker, returning original');
      return { dataUrl: imageData, enhanced: false };
    }
  ...
  async convertFormat(imageData, format = 'jpeg', quality = 0.9) {
    // In service worker context, return original image
    if (!this.canProcessImages()) {
      console.log('🔧 Format conversion not available in service worker, returning original');
      return imageData;
    }
  ...
  async extractColors(imageData, colorCount = 5) {
    // Debug logging for service worker context
    console.log('🔍 extractColors called - context check:', {
      isServiceWorker: this.isServiceWorker,
      hasCanvas: !!this.canvas,
      hasImage: typeof Image !== 'undefined',
      canProcess: this.canProcessImages()
    });
    
    // In service worker context, return default colors
    if (!this.canProcessImages() || typeof Image === 'undefined') {
      console.log('🔧 Returning default colors in service worker context');
      return ['#2c3e50', '#3498db', '#e74c3c', '#f39c12', '#27ae60'].slice(0, colorCount);
    }
  ...
  async validateImage(imageData) {
    // In service worker context, provide basic validation
    if (!this.canProcessImages()) {
      console.log('🔧 Basic validation in service worker context');
      return {
        valid: true,
        width: 800,
        height: 600,
  ...
  async getImageMetadata(imageData) {
    // In service worker context, provide basic metadata
    if (!this.canProcessImages()) {
      console.log('🔧 Providing basic metadata in service worker context');
      return {
        valid: true,
        width: 800,
        height: 600,
        ...
        format: this.getImageFormat(imageData) || 'jpeg'
      };
    }
